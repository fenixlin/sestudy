  <div id="content" class="main box container-fluid">
    <div class="row-fluid">
      <div id="sidecontent" class="span3">
        <div id="sidebar">
          <ul class="nav nav-list">
            ...
            <li class="divider"></li>
            <li class="active"><a href="<?=site_url()?>slide.html">课件管理</a></li>
	  ...
	  <div id="maincontent" class="span9">
		<a href="#myModal" role="button" class="btn btn-primary btn-lg" data-toggle="modal">上传课件</a>
		<div id="myModal" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
				<h3 id="myModalLabel">上传课件</h3>
			</div>
			
			<form action="/sestudy/index.php/slide/up" method="post" enctype="multipart/form-data">
		...
		<?php $data = $this->slide_model->seetch_course_upload($userid, $courseid);?>
		<?php //var_dump($data); ?>
	    <?php $i = 1; ?>
        <table align="center" class="table table-hover" style="border-left: 1px solid #dddddd;border-right: 1px solid #dddddd;border-bottom: 1px solid #dddddd;border-collapse: separate;-webkit-border-radius: 4px;-moz-border-radius: 4px;border-radius: 4px;">
          <tbody>
			...
			<tr>
			  ...
			  <td>
				<?php $classdata = $this->slide_model->get_time($data[$key]->classid);?>
				<?php //var_dump($data); ?>
				<?=$classdata[0]->time   ?>
			  </td>
			  ...
			  <td><a href="#myModal<?=$key?>" data-toggle="modal"><i class="icon-refresh"></i></a></td>
				...
				<div id="myModal<?=$key?>" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
				  ...
				  <form action="/sestudy/index.php/slide/reup/<?=$data[$key]->filename?>" method="post" enctype="multipart/form-data">
				    &nbsp&nbsp&nbsp&nbsp<input type="file" name="upfile">
				  ...
				  &nbsp&nbsp&nbsp
				<?php $query = $this->account_model->get_teacher_class_list($userid, $courseid); ?>
				<select class="form-control" id="classid" name="classid">
					<?php foreach ($query->result() as $row) { ?>
					<option value=<?=$row->classid ?>><?=$row->time ?></option>
					<?php } ?>
			  ...
			  <td><a href="javascript:HandleOnClose('<?=site_url()?>slide/delete_slide/<?=$data[$key]->filename?>')"><i class="icon-remove"></i></a></td>
            </tr>
			<?php } ?>
			</tbody>
        </table>
