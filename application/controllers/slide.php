...
class Slide extends CI_Controller 
{
	...
	public function index()
	{
		...
		else if ($role == "A")
		{
            ...
            $this->load->view('assistant/slide');
            $this->load->view('footer');
		}
	...
	public function up()
	{
		...
		if(!empty($_POST['sub']))
		{
			...
			if($file['size'] >= 20000000)  //20M
			{
	            ...
	            echo "<script>location.href='/sestudy/index.php/slide';</script>";
			}
			...
			else
			{
				...
				move_uploaded_file($file['tmp_name'],"./upload/slide/{$up_time}{$filename2}");
				...
				$this -> slide_model->slide_insert($arr);
	...
	{
		$this->slide_model->delete_file($filename);
		chmod(dirname(dirname(dirname(__FILE__)))."/upload/slide", 0777);  //更改权限
		unlink(dirname(dirname(dirname(__FILE__)))."/upload/slide/".$filename);
		$this -> up();
	}
	
	public function delete_slide($filename)
	{
		$this->slide_model->delete_file($filename);
		chmod(dirname(dirname(dirname(__FILE__)))."/upload/slide", 0777);  //更改权限
		unlink(dirname(dirname(dirname(__FILE__)))."/upload/slide/".$filename);
	...
	public function download($file_name)
	{
		...
		$file_sub_path=dirname(dirname(dirname(__FILE__)))."/upload/slide/"; 
		...
		$this -> slide_model->douwncount_plus($file_name);
		fclose($fp); 
	}
